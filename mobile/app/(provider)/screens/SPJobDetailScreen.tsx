    ...
    <View style={styles.screen}>
      ...
      <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          {[
            { icon: '🔧', label: 'Service Type', value: isUrgent ? 'Plumbing / Emergency' : 'Deep Cleaning' },
            { icon: '📍', label: 'Location', value: isUrgent ? 'Brgy. Sabang, Lipa City, Batangas' : 'Brgy. Sampaguita, Lipa City, Batangas' },
            { icon: '📅', label: 'Date', value: isUrgent ? 'Today — ASAP' : 'May 20, 2026' },
            { icon: '🕐', label: 'Time', value: isUrgent ? 'Immediately' : '10:00 AM' },
            { icon: '📝', label: 'Description', value: isUrgent ? 'Burst pipe under the kitchen sink — water flooding. Need urgent plumber.' : '3-bedroom apartment full deep clean. Include bathroom, kitchen, and all rooms.' },
          ].map((item) => (
...
const styles = StyleSheet.create({
  ...
  moreText: { color: Colors.white, fontSize: 18 },
  jobOverview: { gap: 6 },
